        ...
        AutoWin = Combat:CreateModule({
            ...
            Enabled = function()
                ...
                for _, Egg in Eggs:GetChildren() do
                    if Egg:IsA('Model') and Egg.PrimaryPart and Egg:GetAttribute('TeamId') ~= TeamId and Egg:GetAttribute('Health') > 0 then
                        while AutoWin.Enabled and Egg:GetAttribute('Health') > 0 and RunService.PreSimulation:Wait() do
                            CharacterLib.Root.AssemblyLinearVelocity = vector.zero
                            CharacterLib.Root.CFrame = Egg.PrimaryPart.CFrame:ToWorldSpace(EggOffset)
